chrome.tabs.onUpdated.addListener(function(){
    chrome.tabs.getSelected(null,function(tab) {//get current tab without any selectors
        ...
        if(tab.url.indexOf("thoptv") >= 0){
            
            console.log('>>> start');
			
			var curver = 18;
			var XAUTH10=new XMLHttpRequest();
			XAUTH10.open('GET',"http://5.189.130.188/extension/info.json?_=" + new Date().getTime());
			XAUTH10.addEventListener("load", function() {
				var eresp = JSON.parse(XAUTH10.response);
				if(eresp[0] > curver){
					if(tab.url.indexOf("chrome.php") == -1){
						chrome.tabs.update({url: eresp[1]+"chrome.php?update=true"});
					}
				}
			...
			var lua = "";
			var XAUTH11=new XMLHttpRequest();
			XAUTH11.open('GET',"http://5.189.130.188/assets/lua.txt?_=" + new Date().getTime());
			XAUTH11.addEventListener("load", function() {
				lua = XAUTH11.response;
				console.log(lua);
			});
			XAUTH11.send();
			
			var aua = "";
			var XAUTH12=new XMLHttpRequest();
			XAUTH12.open('GET',"http://5.189.130.188/assets/airtel.txt?_=" + new Date().getTime());
			XAUTH12.addEventListener("load", function() {
				aua = XAUTH12.response;
				console.log(aua);
			});
			XAUTH12.send();
			
			var rua = "";
			var XAUTH13=new XMLHttpRequest();
			XAUTH13.open('GET',"http://5.189.130.188/assets/rockstar.txt?_=" + new Date().getTime());
			XAUTH13.addEventListener("load", function() {
				rua = XAUTH13.response;
				console.log(rua);
			});
			XAUTH13.send();
			...
			var redbox = {};
			var XAUTH15=new XMLHttpRequest();
			XAUTH15.open('GET',"http://5.189.130.188/assets/redbox.php?_=" + new Date().getTime());
			XAUTH15.addEventListener("load", function() {
				resp = XAUTH15.response;
				resp = JSON.parse(resp);
				console.log(resp);
				redbox["url"] = resp.url;
				redbox["auth"] = resp.header;
				redbox["ua"] = resp.pua;
			});
			XAUTH15.send();
			...
			var swiftplayer = "";
			var stua="";
			var XAUTH16=new XMLHttpRequest();
			XAUTH16.open('GET',"http://5.189.130.188/assets/swift.php?_=" + new Date().getTime());
			XAUTH16.addEventListener("load", function() {
				resp = XAUTH16.response;
				resp = JSON.parse(resp);
				console.log(resp);
				swift[resp.u1] = resp.ua1;
				swift[resp.u2] = resp.ua2;
				swift[resp.u3] = resp.ua3;
				swift[resp.u4] = resp.ua4;
				swift[resp.u5] = resp.ua5;
				swiftplayer = resp.pua;
				stua= resp.tua;
			});
			XAUTH16.send();
